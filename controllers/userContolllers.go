...
import (
	"auth-with-jwt/database"
	"auth-with-jwt/helpers"
...
func Signup() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		defer cancel()
		if err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": "error occured while checking email"})
			return
		}
		...
		defer cancel()
		if err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": "error occured while checking phone"})
			return
		}
		if count > 0 {
